...
impl GSplatData {
    ...
    pub fn from_float32_arrays(
        ...
        Ok(GSplatData::new(
            positions, scales, rotations, colors, model_view, count,
        ))
    }
...
unsafe fn reorder_attributes_simd(data: &mut GSplatData, indices: &[u32]) {
    ...
    for (new_idx, &old_idx) in indices.iter().enumerate() {
        ...
        if old_idx * 3 + 2 >= data.positions.len() || new_idx * 3 + 2 >= new_positions.len() {
            break;
        }
        ...
        if old_idx * 3 + 2 >= data.scales.len() || new_idx * 3 + 2 >= new_scales.len() {
            break;
        }
        ...
        if old_idx * 4 + 3 >= data.rotations.len() || new_idx * 4 + 3 >= new_rotations.len() {
            break;
        }
        ...
        if old_idx * 4 + 3 >= data.colors.len() || new_idx * 4 + 3 >= new_colors.len() {
            break;
        }
...
pub fn radix_sort_simd(data: &mut GSplatData) -> Result<(), JsValue> {
    ...
    let mut depths = unsafe { compute_depths_simd(&data.positions, &data.model_view, count) };
...
unsafe fn count_frequencies_simd(depths: &[i32], shift: u32, counts: &mut [u32; 256]) {
    unsafe {
        ...
        for chunk in depths.chunks_exact(4) {
            ...
            let bytes = v128_and(shifted, mask);
